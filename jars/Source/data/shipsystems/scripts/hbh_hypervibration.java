...
public class hbh_hypervibration extends BaseShipSystemScript {
	public static final Object KEY_SHIP = new Object();
	public static final Object KEY_TARGET = new Object();
	
	public static final float DAM_MULT = 2f;
	public static final float RANGE = 300f;
	
	public static final Color TEXT_COLOR = new Color(255,255,255,255);
	
	public static final Color JITTER_COLOR = new Color(138,3,3,100);
	public static final Color JITTER_UNDER_COLOR = new Color(138,3,3,200);
	...
	public void apply(MutableShipStatsAPI stats, final String id, State state, float effectLevel) {
		...
		if (state == State.IN && targetDataObj == null) {
			...
			if (target != null) {
				if (target.getFluxTracker().showFloaty() || 
					...
					target.getFluxTracker().showOverloadFloatyIfNeeded("Hyper-Vibrated!", TEXT_COLOR, 4f, true);
				}
			}
		...
		if (effectLevel > 0) {
			...
			if (targetJitterLevel > 0) {
				...
				targetData.target.setJitter(KEY_TARGET, color, targetJitterLevel, 3, 0, 0 + jitterRangeBonus);
				targetData.target.setJitterUnder(KEY_TARGET, color, targetJitterLevel, 25, 0f, 7f + jitterRangeBonus);
			}
		}
	}
