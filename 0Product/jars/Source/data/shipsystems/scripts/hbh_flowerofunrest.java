...
//					cloak.getSpecAPI().getIconSpriteName(), cloak.getDisplayName(), "can not be hit", false);
			...
			Global.getCombatEngine().maintainStatusForPlayerShip(STATUSKEY2,
					cloak.getSpecAPI().getIconSpriteName(), cloak.getDisplayName(), "improved maneuverability", false);
			Global.getCombatEngine().maintainStatusForPlayerShip(STATUSKEY3,
					cloak.getSpecAPI().getIconSpriteName(), cloak.getDisplayName(), "+" + (int)SPEED_BONUS + " top speed", false);
		} else {
...
//					spec.getIconSpriteName(), cloak.getDisplayName(), "damage mitigated by " + (int) percent + "%", false);
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (player) {
			...
			stats.getDeceleration().modifyPercent(id, SPEED_BONUS * 10f * effectLevel);
